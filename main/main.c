...
static void rx_task(void *arg)
{
    ...
    while (1) {
        ...
        if (rxBytes > 0) {
            ...
            esp_spp_write(spp_conn_handle, rxBytes, data);
...
void sendData(const char* logName, const char* data)
{
 ...
 static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
 {
     ...
     case ESP_SPP_DATA_IND_EVT:
         ...
         char *incoming = malloc(len + 1);
         memcpy(incoming, param->data_ind.data, len);
         incoming[len] = '\0';
         
         int start = 0;

         while (start < len && !isprint((unsigned char)incoming[start])) {
            start++;
        }

         //sendData(SPP_TAG, (const char*)param->data_ind.data);
         sendData(SPP_TAG, &incoming[start]);
